...
function drawTree() {
  ...
  ctx.strokeStyle = "rgba(74, 73, 68, 0.22)";
  drawBranch([[350, 835], [330, 695], [322, 565], [312, 415], [330, 240], [360, 92]], 2.2);
  drawBranch([[327, 580], [260, 498], [180, 430], [108, 350], [78, 274]], 1.5);
  drawBranch([[312, 470], [392, 412], [478, 348], [570, 260], [632, 205]], 1.5);
  drawBranch([[334, 665], [275, 620], [190, 584], [92, 548], [42, 500]], 1.1);
  drawBranch([[338, 690], [412, 660], [506, 618], [640, 558], [688, 505]], 1.1);
  drawBranch([[318, 410], [282, 344], [238, 282], [182, 220], [150, 170]], 1.1);
  drawBranch([[325, 365], [365, 310], [435, 238], [500, 158], [532, 96]], 1.1);

  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  for (const [a, b] of memoryLinks) {
    const from = branchSeeds[a];
    const to = branchSeeds[b];
    const pulse = Math.sin(t * 1.7 + a * 0.41 + b * 0.23) * excitation;
    ctx.strokeStyle = `rgba(74, 73, 68, ${0.12 + Math.max(0, pulse) * 0.12})`;
    ctx.lineWidth = 0.8 + Math.max(0, pulse) * 0.45;
    ctx.beginPath();
  ...
  for (let i = 0; i < branchSeeds.length; i++) {
    ...
    const breathe = Math.sin(t * 1.4 + i * 0.72) * (0.45 + excitation * 0.9);
    const isCore = i === 2 || i === 10 || i === 14 || i === 23;
    const radius = (isCore ? 4.2 : 2.3) + breathe;

    ctx.fillStyle = isCore ? "rgba(184, 78, 63, 0.64)" : "rgba(48, 48, 45, 0.62)";
    ctx.strokeStyle = isCore ? "rgba(184, 78, 63, 0.18)" : "rgba(48, 48, 45, 0.14)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(sx(p[0]), sy(p[1] + Math.sin(t + i) * excitation * 2.5), sx(radius), 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    if (isCore) {
      ctx.beginPath();
      ctx.arc(sx(p[0]), sy(p[1]), sx(radius + 7 + excitation * 3), 0, Math.PI * 2);
      ctx.stroke();
    }
  }
